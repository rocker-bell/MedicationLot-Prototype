...
// import React, { useState, useEffect, useCallback } from 'react';
// import { useNavigate } from 'react-router-dom';
// // 🟢 FIX 1: Import all necessary components from ethers
...
// // 🟢 ACCEPT PROPS: setSigner, setUserData, setMessage, userData, signer, contract
// function Dashboard({ setUserData, setMessage, userData, signer, contract }) {
...
//     // This logic belongs in App.jsx, which passes the props down.
...
//     useEffect(() => {
//         const checkContractConfig = async () => {
//             // Check if the contract prop is valid AND has an interface (is initialized)
//             if (contract && contract.interface) {
//                 try {
//                     // ✅ FIX 2: Use array access notation to reliably call getter functions
//                     const feeRecipientAddr = await contract["FEE_RECIPIENT"]();
//                     const creditTokenAddr = await contract["creditToken"](); 
...
//         };
//         checkContractConfig();
//     }, [contract, setMessage]); // Dependency is only on the contract object
...
//     // 🚀 FIXED INSERT FUNCTION (ERC-20 APPROVAL)
//     const handleAddProductSubmit = async (e) => {
//         e.preventDefault();
...
//             // Fee for the LotTrackr contract (1 CreditToken with 18 decimals)
//             const fixedFeeAmount = parseUnits("1", 18); // Use parseUnits
...
//             if (allowance < fixedFeeAmount) {
//                 const approvalAmount = parseUnits("1000", 18); // Approve 1000 tokens for convenience
...
//                 disabled={isSubmitting}
//             >
//                 {isSubmitting ? 'Submitting Transaction...' : `Submit Devis (Cost: 1 CreditToken)`}
//             </button>
//             <button 
//                 type="button" 
//                 className="btn-secondary" 
...
// import "../Styles/dashboard.css"; 
// // ABIs are passed in as props (contract) or initialized locally (creditTokenContract)
// import CreditTokenABI from './CreditTokenABI.js'; 

// // 🚨 IMPORTANT: Replace with your actual deployed CreditToken address!
// const CREDIT_TOKEN_ADDRESS = "0x42a05014306386b823329f777eb09ec1f493d69c"; 

// // 🟢 ACCEPT PROPS: contract and creditTokenContract are now passed
...
//     useEffect(() => {
//         const checkContractConfig = async () => {
//             // Wait for the 'contract' prop to be passed down from App.jsx
//             if (contract && contract.interface) {
//                 try {
//                     const feeRecipientAddr = await contract["FEE_RECIPIENT"]();
//                     const creditTokenAddr = await contract["creditToken"](); 
...
//         // 🟢 FIX: Check for 'contract' (LotTrackr) and 'creditTokenContract' props
...
//     // ... (handleCheckProductSubmit is unchanged)
//     const handleCheckProductSubmit = async (e) => {
//         e.preventDefault();
...
// import React, { useState, useEffect, useCallback } from 'react';
// import { useNavigate } from 'react-router-dom';
// import { ethers, Contract, formatUnits, parseUnits } from 'ethers'; // Make sure Ethers and utilities are imported
// import "../Styles/dashboard.css"; 
// // Import ABIs
// import LotTrackerABI from "./LotTrackerABI.js";
// import CreditTokenABI from './CreditTokenABI.js'; 
...
// // 🚨 IMPORTANT: Replace with your actual deployed CreditToken address!
// // This address must match the one used in your App.jsx connection logic
// const CREDIT_TOKEN_ADDRESS = "0x42a05014306386b823329f777eb09EC1f493D69c"; 

// /**
//  * Dashboard Component
//  * Assumes 'userData', 'signer', and 'contract' (the LotTrackr contract) 
//  * are passed as props from the parent component (App.jsx).
//  */
// function Dashboard({ setUserData, setMessage, userData, signer, contract, creditTokenContract }) {
...
//     // State for debugging contract config
//     const [feeRecipient, setFeeRecipient] = useState('...'); 
//     const [contractCreditTokenAddress, setContractCreditTokenAddress] = useState('...');
...
//         const checkContractConfig = async () => {
//             // Wait for the 'contract' prop to be passed down from App.jsx
//             // Check if contract and contract.interface exist to prevent race condition errors
...
//         };
//         checkContractConfig();
//     // Re-run if the contract object (passed as a prop) changes
//     }, [contract, signer, setMessage]);


//     const handleLogout = () => {
//         if (setUserData) setUserData(null); // This triggers localStorage cleanup in App.jsx
//         if (setMessage) setMessage("Successfully logged out. Connect or Register to continue.");
//         navigate("/", { replace: true }); 
//     };
...
//     const formatKey = (key) => {
//         // Updated status map based on Solidity Enum order
//         const statusMap = { '0': 'Active', '1': 'Inactive', '2': 'Transferred', '3': 'Discarded' }; 
//         switch (key) {
//             case 'lotId': return 'Lot ID';
//             case 'name': return 'Product Name';
...
//             // We use the creditTokenContract prop passed from App.jsx
//             const allowance = await creditTokenContract.allowance(signer.address, lotTrackrAddress);
...
//             // Use the 'contract' prop (LotTrackr)
//             const tx = await contract.insert(
//                 newProductData.name,
//                 newProductData.manufacturer,
//                 ethers.toBigInt(timestampInSeconds),
...
//         // 🚨 CRITICAL CONDITIONAL RENDER: Block access if the wallet is not connected.
//         // This waits for the 'signer' prop from App.jsx to be ready.
//         if (!signer || !userData?.address) {
//              return (
//                  <div className="connection-required">
//                      <h2>Connecting Wallet...</h2>
//                      <p>Restoring your session and connecting to the Hedera Testnet.</p>
//                      <p style={{marginTop: '15px', color: '#888'}}>
...
// import { ethers, Contract, BrowserProvider, formatUnits, parseUnits } from 'ethers'; 
// import "../Styles/dashboard.css"; 
// // ABIs are no longer needed here if passed as props or initialized in App.jsx
// // import LotTrackerABI from "./LotTrackerABI.js";
// import CreditTokenABI from './CreditTokenABI.js'; 
...
// // 🟢 FIX 1: Renamed the 'contract' prop to 'lotTrackerContract' for clarity
// function Dashboard({ setUserData, setMessage, userData, signer, lotTrackerContract, creditTokenContract }) {
...
//     useEffect(() => {
//         const checkContractConfig = async () => {
//             // 🟢 FIX 2: Use 'lotTrackerContract' prop
//             if (lotTrackerContract && lotTrackerContract.interface && signer) {
//                 try {
//                     // ✅ FIX: Use array access notation to reliably call getter functions
//                     const feeRecipientAddr = await lotTrackerContract["FEE_RECIPIENT"]();
//                     const creditTokenAddr = await lotTrackerContract["creditToken"](); 
...
//     }, [lotTrackerContract, signer, setMessage]);
...
//         // 🟢 FIX 4: Check for 'lotTrackerContract'
//         if (!lotTrackerContract || !creditTokenContract || !signer) {
//             setMessage("Error: Wallet not connected or Contracts not initialized. Please connect your wallet first.");
//             return; 
//         }
...
//         try {
//             // 1. Resolve Addresses and Prepare Amounts
//             const lotTrackrAddress = await lotTrackerContract.getAddress(); 
//             const timestampInSeconds = dateToUnixTimestamp(newProductData.manufactureDate);
...
//             // 🟢 FIX 5: Use 'lotTrackerContract' to call insert
//             const tx = await lotTrackerContract.insert(
//                 newProductData.name,
//                 newProductData.manufacturer,
//                 ethers.toBigInt(timestampInSeconds),
//                 newProductData.initialLocation,
...
//         // 🟢 FIX 6: Check for 'lotTrackerContract'
//         if (!lotTrackerContract) {
//             setMessage("Error: Contract not initialized. Please connect your wallet first.");
//             return;
//         }
...
//         try {
//             // 🟢 FIX 7: Use 'lotTrackerContract' to call pull
//             const [lotStruct, historyArray] = await lotTrackerContract.pull(ethers.toBigInt(tokenId));
...
//         // 🟢 FIX 8: Check for 'lotTrackerContract' in the loading guard
//         if (!signer || !lotTrackerContract || !userData?.address) {
//              return (
//                  <div className="connection-required">
//                      <h2>Reconnecting Wallet...</h2>
//                      <p>Restoring your session and connecting to the Hedera Testnet. Please wait...</p>
